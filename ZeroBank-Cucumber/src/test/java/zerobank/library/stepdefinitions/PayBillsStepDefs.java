...
import zerobank.library.pages.PayBillsPage;
...
public class PayBillsStepDefs {
    ...
    public void i_complete_a_successful_Pay_operation() {
        Select payeeOpts = new Select(payBillsPage.payeeOpts);

        int rand1 = random.nextInt(payeeOpts.getOptions().size());
        payeeOpts.selectByIndex(rand1);

        Select accountOptions = new Select(payBillsPage.accOptions);
        int rand2 = random.nextInt(accountOptions.getOptions().size());
        accountOptions.selectByIndex(rand2);

        int amountRnd = random.nextInt(100000);
        payBillsPage.amountInput.sendKeys(amountRnd + "");
        ...
        payBillsPage.descrInput.sendKeys("Sending money");
    ...
    public void message_should_be_displayed(String expectedMessage) {
        ...
        if (expectedMessage.contains("success")) {
            ...
            Assert.assertEquals("Alert message does NOT match: " + actualMessage, expectedMessage, actualMessage);
        } else {
            if (payBillsPage.amountInput.getAttribute("value").isEmpty()) {
                ...
                Assert.assertEquals("Alert Box Message does not match",expectedMessage,actual);
            } else {
                String actual = payBillsPage.dateInput.getAttribute("validationMessage");
                Assert.assertEquals("Alert Box Message does not match",expectedMessage, actual);
            }
        }
    }
    ...
    public void i_try_to_make_a_payment_without_entering_the_amount_or_date() {
        ...
        Select payeeOptions = new Select(payBillsPage.payeeOpts);
    ...
    public void following_currencies_should_be_available(List<String> expectedCurrencyList) {
        WebElement element;
        Select listSelect = new Select(payBillsPage.currList);
    ...
    public void user_tries_to_calculate_cost_without_selecting(String enterValueTo) {
        ...
        if (enterValueTo.equalsIgnoreCase("currency")){
            ...
            Select allCurrency = new Select(payBillsPage.currList);
